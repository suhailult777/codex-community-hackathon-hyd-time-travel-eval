...
- **Scenario Generation via LLMs**: Uses structured-output capable providers such as NVIDIA-hosted models and OpenAI models to automatically generate diverse, plausible failure mode scenarios based solely on a high-level task and environment metadata.
...
2. Obtain a **NVIDIA API Key**, an **OpenAI API Key**, or both.
...
1. Copy `.env.example` to `.env` and fill in the provider keys you want to use:

```bash
cp .env.example .env
```

2. Set your keys and choose the default provider:
...
Use the sidebar `Provider` dropdown to switch between NVIDIA and OpenAI at runtime.
...
python main.py --provider openai --task "Deploy v2 of the frontend application to production"
